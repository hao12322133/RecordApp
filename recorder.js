...
const recording = {
    ...
    async  processRecording() {
        ...
        const inputFileName = 'recording.webm';  // Tên file ghi lại
        const outputFileName = 'output.mp4';    // Tên file đầu ra

        // Sử dụng fetchFile để tải tệp vào bộ nhớ FFmpeg
        ffmpeg.FS('writeFile', inputFileName, await fetchFile(state.recordedBlob));

        // Chạy lệnh FFmpeg để chuyển đổi tệp
        await ffmpeg.run('-i', inputFileName, '-c:v', 'libx264', '-preset', 'fast', '-crf', '23', '-c:a', 'aac', '-b:a', '128k', outputFileName);

        // Đọc file đầu ra
        const data = ffmpeg.FS('readFile', outputFileName);

        // Chuyển đổi dữ liệu thành Blob
        const recordedBlob = new Blob([data.buffer], { type: 'video/mp4' });

        // Xử lý Blob nếu cần (ví dụ, tải xuống, chia sẻ)
    }
};
const videoExport = {
    download() {
